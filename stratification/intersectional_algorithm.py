...
from stratification.utils import AverageMeter, compute_accuracy, compute_roc_auc
# from .datasets import LABEL_TYPES, GEORGEDataset
from stratification.classification.losses.loss_computer import LossComputer
...
def init_criterion(num_subclasses, subclass_counts, criterion_config, robust, trainset, use_cuda):
...
class ALGOClassification:
    ...
    def train(self, model, train_dataloader, val_dataloader, classifiers, robust=False):
        ...
        for epoch in range(num_epochs):
            ...
            train_metrics, _ = self._run_epoch(model, train_dataloader, classifiers, optimize=True,
                                               save_activations=False)
            self.logger.basic_info('Validation:')
            val_metrics, _ = self._run_epoch(model, val_dataloader, classifiers, optimize=False,
    ...
    def _run_epoch(self, model, dataloader, classifiers, optimize=False, save_activations=False, reweight=None,
        ...
        for batch_idx, (inputs, targets) in enumerate(dataloader):
            ...
            for classifier in classifiers:
                classifier_results = torch.argmax(classifier(inputs), 1)
                if loss_targets == None:
                    loss_targets = classifier_results
                else:
                    loss_targets = ((classifier_results + loss_targets) > 0) + 0
            target_idx = loss_targets # will need to change
            ...
            if optimize and not bit_pretrained:
                ...
                co = self.criterion(logits, targets, target_idx)
            ...
            reweight_vec = None if reweight is None else reweight[target_idx] # might fail
            ...
            outputs['targets'].append(targets.detach().cpu())
