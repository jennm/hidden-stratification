...
class FindEmbeddings():
    ...
    def update_model(self, model):
        self.model = model
        self.model.eval()
        get_hooks(self.model)

    def get_example_embeddings(self, example, y_true, embedding_num, model_type='res_net'):
        if  model_type == 'res_net':
            embedding_num += 2
        output = self.model(example)
        pred = torch.argmax(output, 1)
        pred = pred.to(torch.float)
        y_true_tensor = torch.tensor(y_true, dtype=torch.float)
        loss = self.criterion(pred, y_true_tensor)
        embeddings = {'embedding': tail_cache[embedding_num], 'loss': loss, 'predicted_label': pred}
        del tail_cache
        tail_cache = dict()
        return embeddings
...
def main():
    ...
    parser.add_argument('--data_info', type=str, help='column names in data_info')
